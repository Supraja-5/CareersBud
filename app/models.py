...
# =============================================================================
# ADD TO END OF app/models.py
# =============================================================================

# User Settings Models - Add to end of your existing models.py file
class UserSettings(db.Model):
    ...
    desired_roles = db.Column(db.JSON, default=list)  # Array of desired job roles
    preferred_locations = db.Column(db.JSON, default=list)  # Array of preferred locations
    ...
    job_listing_languages = db.Column(db.JSON, default=list)  # Array of preferred languages
    ...
    def update_from_dict(self, data):
        ...
        for key, value in data.items():
            if hasattr(self, key) and key not in ['id', 'user_id', 'created_at']:
                # Handle JSON fields
                if key in ['desired_roles', 'preferred_locations', 'job_listing_languages', 'connected_accounts', 'webhook_events']:
                    if isinstance(value, str):
                        import json
                        try:
                            value = json.loads(value)
                        except:
                            value = [] if key.endswith('s') else {}
                
                # Handle time fields
                elif key in ['quiet_hours_start', 'quiet_hours_end'] and value:
                    try:
...
class UserSession(db.Model):
    ...
    def __repr__(self):
        return f'<UserSession {self.user_id}:{self.device_info[:20] if self.device_info else "Unknown"}>'
...
def export_user_data(user_id):
    ...
    if hasattr(user, 'user_settings') and user.user_settings:
        data['settings'] = user.user_settings.to_dict()
...
def delete_user_account(user_id, confirmation_text):
    ...
    try:
        ...
        # This will cascade delete most other related data
